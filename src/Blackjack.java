...
public class Blackjack implements Game {
    ...
    public boolean printOneTurn() {
        if (turn == 1)
            Collections.shuffle(dealerCard);
        ...
        if (playerCard.get(0).getNum() == playerCard.get(1).getNum() && turn == 1) {
            ...
            if (c == 1) {
                p2Card.add(playerCard.get(0));
                p2Card.add(blackDeck.popOneCard());
                playerCard.set(0, blackDeck.popOneCard());
                splitCard = true;
                Main.clearScreen();
        ...
        while (turn != 9) {
            ...
            if (splitCard && sec == false) {
                ...
                if (hs == 1) {
                    ...
                    System.out.println("Your new second deck Card : " + newCard.getShape() + "\t");
                    newCard.print();
                    System.out.println();
                } else if (hs == 2) {
                    sec = true;
    ...
    public void printResult(ArrayList<Card> dl, ArrayList<Card> pl) {
        ...
        if (bust1) {
            ...
            if (sumOfNum(dl) > 21) {
                ...
                int dScore = 21 - sumOfNum(dl);
                int pScore = 21 - sumOfNum(pl);
                if (dScore <= pScore) {
        ...
        if (insuBool) {
            betMoney -= Math.ceil(tempBetMoney * 0.5);
        }
    }
