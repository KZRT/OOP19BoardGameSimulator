...
public class HandScorer {
    ...
    private int findFlush() {
        ...
        int ans = -1;
        for (Card c : hand) {
            CardShape tempSuit = c.getShape();
            int currSuit =0;
            switch(tempSuit)
            {
                case SPADE: currSuit=0; break;
                case HEART: currSuit=1; break;
                case DIAMOND: currSuit=2; break;
                case CLOVE: currSuit=3; break;
            }
            int currNum = c.getNum();
            if (currNum == 1) {
            ...
            int highCardOfThisSuit = highestCard[currSuit];
            if (currNum > highCardOfThisSuit) {
                highestCard[currSuit] = currNum;
            }
            if (++numSuit[currSuit] >= 5) {
                // We have a flush!
                ans = highestCard[currSuit];
            }
        }
        return ans;
    }
