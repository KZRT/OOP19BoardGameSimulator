...
public class Taisai implements Game {
	...
	public boolean nextTurn(Scanner input) {
		...
		if (!isGameEnd()) {
			...
			System.out.println("Total Bet Money = " + totalBetMoney);
			System.out.println("Total Earn Money = " + totalEarnMoney);
			System.out.println("Total Lose Money = " + totalLoseMoney);
			return false;
		}
		return true;
	}
	...
	private void rollDice() {
		...
		for (int i = 0; i < 3; i++) {
			taisaiDice[i].roll();
			System.out.print(taisaiDice[0].getDiceNum() + " ");
		}
		System.out.println();
	}
